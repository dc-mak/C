...
void Database_set(struct Connection *conn,
		int id, const char *name, const char *email, const char *phone)
{
...
int main(int argc, char *argv[])
{
	...
	if (argc > 3) id = atoi(argv[3]); // strtoI ideally
	if (id >= MAX_ROWS) die(conn, "Incorrect record number.");
	...
	switch(action) {
		...
		case 'f':
			if (argc != 4)
				die(conn, "Need either name, email or phone");
